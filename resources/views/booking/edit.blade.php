...
@section('content')
                            ...
                            <select class="form-control @error('service_id') is-invalid @enderror" id="service_id"
                                ...
                                @foreach ($services as $service)
                                    <option value="{{ $service->id }}"
                                        {{ old('service_id', $booking->services) == $service->id ? 'selected' : '' }}>
                                        {{ $service->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="number" class="form-control @error('no_of_guest') is-invalid @enderror"
                                id="no_of_guest" name="no_of_guest" value="{{ old('no_of_guest', $booking->no_of_guest) }}"
                                min="1" required>
                            @error('no_of_guest')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                            ...
                            <select class="form-control @error('promotion_id') is-invalid @enderror" id="promotion_id"
                                ...
                                @foreach ($promotions as $promotion)
                                    <option value="{{ $promotion->id }}"
                                        {{ old('promotion_id', $booking->promotions) == $promotion->id ? 'selected' : '' }}>
                                        {{ $promotion->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select class="form-control @error('sales_agent_id') is-invalid @enderror" id="sales_agent_id"
                                ...
                                @foreach ($salesAgents as $agent)
                                    <option value="{{ $agent->id }}"
                                        {{ old('sales_agent_id', $booking->sales_agents) == $agent->id ? 'selected' : '' }}>
                                        {{ $agent->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control @error('booking_agent') is-invalid @enderror"
                                ...
                                value="{{ old('booking_agent', $booking->booking_agent) }}" required>
                            @error('booking_agent')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="number" step="0.01"
                                ...
                                name="deposit_amount" value="{{ old('deposit_amount', $booking->deposit_amount) }}"
                                min="0" required>
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <input type="number" step="0.01"
                                ...
                                name="sales_amount" value="{{ old('sales_amount', $booking->sales_amount) }}"
                                min="0" required>
                            @error('sales_amount')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
 ...
 @php
    ...
    $selectedStart = old('start_time', isset($booking->start_time) ? Carbon::parse($booking->start_time)->format('H:i') : null);
    $selectedEnd = old('end_time', isset($booking->end_time) ? Carbon::parse($booking->end_time)->format('H:i') : null);
@endphp
        ...
        <select class="form-control @error('start_time') is-invalid @enderror" name="start_time" id="start_time" required>
            @for ($i = 9; $i <= 16; $i++)
                @php
                    $hour = sprintf('%02d', $i);
                @endphp
                <option value="{{ $hour }}:00" {{ $selectedStart == "$hour:00" ? 'selected' : '' }}>{{ $hour }}:00</option>
                <option value="{{ $hour }}:30" {{ $selectedStart == "$hour:30" ? 'selected' : '' }}>{{ $hour }}:30</option>
            @endfor
            <option value="17:00" {{ $selectedStart == "17:00" ? 'selected' : '' }}>17:00</option>
        </select>
        @error('start_time')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
        ...
        <select class="form-control @error('end_time') is-invalid @enderror" name="end_time" id="end_time" required>
            @for ($i = 9; $i <= 16; $i++)
                @php
                    $hour = sprintf('%02d', $i);
                @endphp
                <option value="{{ $hour }}:00" {{ $selectedEnd == "$hour:00" ? 'selected' : '' }}>{{ $hour }}:00</option>
                <option value="{{ $hour }}:30" {{ $selectedEnd == "$hour:30" ? 'selected' : '' }}>{{ $hour }}:30</option>
            @endfor
            <option value="17:00" {{ $selectedEnd == "17:00" ? 'selected' : '' }}>17:00</option>
        </select>
        @error('end_time')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
